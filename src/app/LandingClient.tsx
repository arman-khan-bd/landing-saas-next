...
import {
  ...
  Target, Award, Users, Heart, Search, ChevronDown, ChevronUp
} from "lucide-react";
...
export default function LandingClient({ initialPlans, initialFeatures }: LandingClientProps) {
  ...
  const defaultFeatures = [
    { icon: Layout, title: "ল্যান্ডিং পেজ বিল্ডার", desc: "ড্র্যাগ এন্ড ড্রপ দিয়ে তৈরি করুন হাই-কনভার্টিং পেজ।", category: "builder" },
    { icon: Smartphone, title: "OTP ভেরিফিকেশন", desc: "SMS এর মাধ্যমে কাস্টমার ভেরিফাই করুন এবং ফেক অর্ডার রোধ করুন।", category: "sales" },
    { icon: BarChart3, title: "সেলস এনালাইটিক্স", desc: "ব্যবসার রিয়েল-টাইম ডাটা, অর্ডার ও বিক্রির গ্রাফ দেখুন।", category: "sales" },
    ...
    { icon: ShieldCheck, title: "ব্যাংক লেভেল সিকিউরিটি", desc: "আপনার ডাটা ও ট্রানজাকশন সবসময় সুরক্ষিত থাকবে।", category: "admin" },
    { icon: Sparkles, title: "এআই ডেসক্রিপশন জেনারেটর", desc: "Gemini AI ব্যবহার করে সেকেন্ডেই আকর্ষণীয় ডেসক্রিপশন তৈরি করুন।", category: "builder" },
    { icon: Rocket, title: "সুপার ফাস্ট পেজ স্পিড", desc: "গতি অপ্টিমাইজড পেজ যা গ্রাহকের কেনাকাটার অভিজ্ঞতা বাড়ায়।", category: "builder" },
    { icon: ShoppingCart, title: "স্মার্ট পেমেন্ট ও কার্ট", desc: "বিকাশ, নগদ, রকেটের পাশাপাশি ক্যাশ অন ডেলিভারি সাপোর্ট।", category: "sales" },
    { icon: TrendingUp, title: "কুপন ও ডিসকাউন্ট", desc: "প্রোমো কোড ও ডিসকাউন্ট অফার দিয়ে বিক্রির পরিমাণ বাড়িয়ে নিন।", category: "sales" },
    ...
    { icon: Users, title: "কাস্টমার ডাটাবেজ", desc: "সব গ্রাহকের তথ্য এক জায়গায় সংরক্ষণ করে রিমার্কেটিং করুন।", category: "admin" }
  ];
    ...
    <div className="flex flex-col items-center min-h-screen bg-slate-50 selection:bg-primary selection:text-white overflow-x-hidden">
      ...
      <main className="w-full">
        ...
        <section id="features" className="py-16 sm:py-24 bg-slate-50/50">
          <div className="max-w-7xl mx-auto px-4 space-y-12 sm:space-y-16">
            ...
            {/* Interactive Feature Explorer when expanded */}
            {showAll && (
              <div className="space-y-8 animate-in fade-in slide-in-from-top-4 duration-500 max-w-5xl mx-auto">
                <div className="flex flex-col md:flex-row items-center gap-4 bg-white p-4 rounded-3xl border border-slate-100 shadow-sm">
                  ...
                  <div className="relative w-full md:flex-1">
                    ...
                    <input
                      type="text"
                      placeholder="ফিচার খুঁজুন (উদাঃ ল্যান্ডিং পেজ, OTP)..."
                      className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:bg-white transition-all"
                      value={searchTerm}
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 w-full md:w-auto">
                    {[
                      { id: "all", label: "সব ফিচার" },
                      { id: "builder", label: "বিল্ডার ও ডিজাইন" },
                      { id: "sales", label: "সেলস ও পেমেন্ট" },
                      ...
                      <button
                        ...
                        className={cn(
                          "px-4 py-2 text-xs font-black uppercase tracking-wider rounded-xl transition-all duration-300",
                ...
                {filteredFeatures.length === 0 && (
                  <div className="text-center py-16 bg-white rounded-3xl border border-slate-100 shadow-inner">
                    <p className="text-sm font-bold text-slate-400">দুঃখিত, কোনো ফিচার খুঁজে পাওয়া যায়নি।</p>
                  </div>
                )}
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 transition-all duration-500">
               {(showAll ? filteredFeatures : currentFeatures.slice(0, 6)).map((f, i) => (
                  <div key={i} className="p-6 sm:p-8 bg-white rounded-3xl border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all group animate-in fade-in duration-500">
              ...
              <div className="max-w-4xl mx-auto bg-white rounded-[32px] sm:rounded-[48px] p-6 sm:p-12 border border-slate-100 shadow-sm space-y-6 sm:space-y-8">
                 <div className="flex items-center justify-between border-b border-slate-50 pb-6">
                    ...
                    <Badge variant="outline" className="text-[10px] font-black uppercase tracking-widest">
                      {showAll ? `${filteredFeatures.length}টি ফিচার` : "সব সুবিধা"}
                    </Badge>
                 </div>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-x-12 sm:gap-y-6">
                    {(showAll ? filteredFeatures : currentFeatures.slice(0, 6)).map((f, i) => (
                       <div key={i} className="flex items-center gap-4 py-3 border-b border-slate-50 last:border-0 md:border-0 md:odd:border-r md:odd:pr-6 animate-in fade-in duration-500">
            ...
            <div className="flex justify-center pt-4">
              <Button
                ...
                size="lg"
                className="h-12 px-8 rounded-full bg-slate-950 text-white font-black text-xs uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-lg flex items-center gap-2"
              >
                {showAll ? (
                  <>
                    ফিচারসমূহ বন্ধ করুন <ChevronUp className="w-4 h-4" />
                  </>
                ) : (
                  <>
                    সকল ফিচার দেখুন <ChevronDown className="w-4 h-4" />
                  </>
                )}
              </Button>
            </div>
        ...
        <section id="pricing" className="py-16 sm:py-24 px-4">
          <div className="max-w-7xl mx-auto space-y-12 sm:space-y-20">
            <div className="text-center space-y-3">
              ...
              <p className="text-slate-500 text-sm sm:text-lg font-medium">আপনার সাধ্যের মধ্যেই সেরা সリューション।</p>
            </div>
