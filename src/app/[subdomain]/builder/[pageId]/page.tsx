...
export default function PageBuilder() {
  ...
  if (loading) return <div className="flex h-screen items-center justify-center bg-slate-900"><Loader2 className="animate-spin text-primary w-12 h-12" /></div>;

  return (
    <div className="flex h-screen bg-[#F1F3F6] overflow-hidden text-slate-800 font-body select-none">
      {/* --- ELEMENTOR ADVANCED SIDEBAR --- */}
      <aside className="w-[360px] h-full bg-[#1e1e2e] text-slate-300 z-50 flex flex-col shrink-0 shadow-[10px_0_30px_rgba(0,0,0,0.1)]">
        <div className="p-5 flex items-center justify-between bg-[#161625] border-b border-white/5">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-primary rounded-xl flex items-center justify-center shadow-lg shadow-primary/20">
              <Sparkles className="w-5 h-5 text-white" />
            </div>
            <div>
              <span className="block font-headline font-bold text-white text-base leading-none">iHut Pro Editor</span>
              <span className="text-[9px] text-primary font-bold uppercase tracking-widest mt-1 block">Advanced Mode</span>
            </div>
          </div>
          <Button variant="ghost" size="icon" onClick={() => router.push(`/${subdomain}/builder`)} className="rounded-full text-slate-500 hover:text-white hover:bg-white/5">
            <ArrowLeft className="w-5 h-5" />
          </Button>
        </div>

        <Tabs value={sidebarTab} onValueChange={(v: any) => setSidebarTab(v)} className="flex-1 flex flex-col pt-2">
          <TabsList className="grid grid-cols-2 p-1 bg-white/5 mx-4 rounded-xl shrink-0 h-11">
            <TabsTrigger value="elements" className="rounded-lg font-bold text-[10px] uppercase tracking-wider data-[state=active]:bg-primary data-[state=active]:text-white">
              <Plus className="w-4 h-4 mr-2" /> Widgets
            </TabsTrigger>
            <TabsTrigger value="edit" className="rounded-lg font-bold text-[10px] uppercase tracking-wider data-[state=active]:bg-primary data-[state=active]:text-white" disabled={!selectedBlockId}>
              <Settings2 className="w-4 h-4 mr-2" /> Controls
            </TabsTrigger>
          </TabsList>

          <div className="flex-1 overflow-y-auto custom-scrollbar px-1 mt-4">
            <TabsContent value="elements" className="mt-0 px-4 space-y-6">
              <div className="space-y-4">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-2">Basic Elements</p>
              ...
              <div className="space-y-4">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-2">Layout & Commerce</p>
                <div className="grid grid-cols-2 gap-2">
            ...
            <TabsContent value="edit" className="mt-0">
              {selectedBlock ? (
                <div className="flex flex-col h-full animate-in fade-in duration-500">
                  <div className="px-5 py-4 bg-white/5 border-y border-white/5 flex items-center justify-between mb-2">
                     <div className="flex items-center gap-3">
                        <div className="p-2 bg-primary/20 rounded-xl text-primary font-bold">
                          {getBlockIcon(selectedBlock.type)}
                        </div>
                        <span className="font-headline font-extrabold text-white text-xs uppercase tracking-tighter">Editing {selectedBlock.type}</span>
                     </div>
                     <span className="text-[9px] font-mono text-slate-600 bg-black/40 px-2 py-1 rounded">ID: {selectedBlock.id}</span>
                  </div>

                  <Tabs defaultValue="content" className="w-full">
                    <TabsList className="w-full bg-[#161625] border-b border-white/5 rounded-none h-12 px-0 gap-0">
                      <TabsTrigger value="content" className="flex-1 rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:text-white font-bold text-[9px] uppercase tracking-widest py-4">Content</TabsTrigger>
                      <TabsTrigger value="style" className="flex-1 rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:text-white font-bold text-[9px] uppercase tracking-widest py-4">Style</TabsTrigger>
                      <TabsTrigger value="advanced" className="flex-1 rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:text-white font-bold text-[9px] uppercase tracking-widest py-4">Advanced</TabsTrigger>
                    </TabsList>
                          ...
                          <div className="space-y-6">
                            ...
                            <div className="space-y-4">
                              <div className="flex justify-between items-center">
                                <Label className="text-[10px] uppercase font-bold text-slate-500 tracking-widest">Font Size</Label>
                           ...
                           <div className="space-y-6">
                              <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-2">
                                  ...
                                  <div className="relative h-10 w-full rounded-xl overflow-hidden border border-white/10 group/color">
                                ...
                                <div className="space-y-2">
                                  <Label className="text-[9px] uppercase font-bold text-slate-500">Background</Label>
                                  <div className="relative h-10 w-full rounded-xl overflow-hidden border border-white/10 group/color">
                      ...
                      <TabsContent value="advanced" className="mt-0 space-y-6 px-1 animate-in slide-in-from-right-4 duration-300">
                        <PropertySection label="Motion Effects" icon={Star}>
                           <div className="space-y-3">
                              ...
                              <Select value={selectedBlock.style?.animation || "none"} onValueChange={(v: any) => updateBlock(selectedBlock.id, { style: { animation: v } })}>
                                <SelectTrigger className="bg-black/20 border-white/5 rounded-xl h-11 text-xs">
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent className="bg-[#1e1e2e] text-slate-300 border-white/10">
                        ...
                        <PropertySection label="Border & Shadow" icon={Settings}>
                           <div className="space-y-6">
                              ...
                              <div className="space-y-3">
                                <Label className="text-[10px] uppercase font-bold text-slate-500 tracking-widest">Box Shadow</Label>
                                <Select value={selectedBlock.style?.boxShadow || "none"} onValueChange={(v: any) => updateBlock(selectedBlock.id, { style: { boxShadow: v } })}>
                                  <SelectTrigger className="bg-black/20 border-white/5 rounded-xl h-11 text-xs"><SelectValue /></SelectTrigger>
                                  <SelectContent className="bg-[#1e1e2e] text-slate-300 border-white/10">
                                    <SelectItem value="none">None</SelectItem>
        ...
        <div className="p-6 bg-[#161625] border-t border-white/5 flex flex-col gap-3">
           <Button className="w-full h-14 rounded-2xl font-bold text-base shadow-2xl shadow-primary/30 group relative overflow-hidden transition-all hover:scale-[1.02]" onClick={handleSave} disabled={saving}>
           ...
           <p className="text-[9px] text-center text-slate-600 font-bold uppercase tracking-[0.2em]">Ready for Production Deployment</p>
        </div>
      </aside>

      {/* --- LIVE VISUAL CANVAS (FOCUS MODE) --- */}
      <main className="flex-1 flex flex-col h-full overflow-hidden relative bg-[#0f0f1a]">
        {/* Designer Header Toolbar */}
        <header className="h-16 bg-[#161625]/80 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-8 shrink-0 z-20">
           <div className="flex items-center gap-6">
              <div className="h-6 w-px bg-white/10 hidden md:block" />
              <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setViewMode("desktop")}>
                <Monitor className={cn("w-5 h-5 transition-colors", viewMode === "desktop" ? "text-primary shadow-[0_0_15px_rgba(59,130,246,0.5)]" : "text-slate-500")} />
                <span className={cn("text-[10px] font-bold uppercase tracking-widest", viewMode === "desktop" ? "text-white" : "text-slate-500")}>Desktop View</span>
              </div>
              <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setViewMode("mobile")}>
                <Smartphone className={cn("w-5 h-5 transition-colors", viewMode === "mobile" ? "text-primary shadow-[0_0_15px_rgba(59,130,246,0.5)]" : "text-slate-500")} />
                <span className={cn("text-[10px] font-bold uppercase tracking-widest", viewMode === "mobile" ? "text-white" : "text-slate-500")}>Mobile Mockup</span>
              </div>
           </div>

           <div className="flex items-center gap-4">
              <div className="hidden lg:flex items-center gap-2 bg-black/40 px-4 py-2 rounded-full border border-white/5 mr-4">
                <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                <span className="text-[10px] font-bold text-slate-400 font-mono tracking-tighter capitalize">{subdomain}.host.live</span>
              </div>
              <Button variant="outline" className="rounded-2xl px-6 h-11 font-bold text-xs bg-transparent border-white/10 text-white hover:bg-white/5 hover:border-primary/50 transition-all" onClick={() => setIsPreviewOpen(true)}>
        ...
        <div className="flex-1 overflow-y-auto p-12 custom-scrollbar-minimal scroll-smooth flex justify-center items-start">
           <div className={cn(
             "transition-all duration-700 bg-white min-h-[100%] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] relative overflow-x-hidden",
             viewMode === "mobile" ? "w-[420px] rounded-[60px] border-[14px] border-[#161625]" : "max-w-6xl w-full rounded-2xl"
           )}>
...
function WidgetButton({ icon: Icon, label, onClick, highlight = false }: any) {
  return (
    <Button 
      ...
      onClick={onClick}
      className={cn(
        "h-[110px] flex-col gap-3 rounded-[28px] border-white/5 bg-white/5 hover:bg-white/10 hover:border-primary/50 transition-all group overflow-hidden",
        highlight ? "border-primary/20 bg-primary/5 shadow-lg shadow-primary/5" : ""
      )}
    >
      <div className={cn(
        "p-3 rounded-2xl bg-[#161625] transition-transform group-hover:scale-110 duration-300", 
        highlight ? "text-primary" : "text-slate-400 group-hover:text-white"
      )}>
        <Icon className="w-7 h-7" />
      </div>
      <span className={cn(
        "text-[10px] font-extrabold uppercase tracking-widest transition-colors",
        highlight ? "text-primary/80" : "text-slate-500 group-hover:text-slate-300"
      )}>{label}</span>
    </Button>
  );
}
...
function AlignButton({ active, icon: Icon, onClick }: any) {
  return (
    <Button 
      variant="ghost" 
      onClick={onClick}
      className={cn("h-10 rounded-xl px-0 translate-y-0 active:translate-y-0.5", active ? "bg-primary text-white shadow-lg shadow-primary/30" : "text-slate-500 hover:text-white hover:bg-white/5")}
    >
...
function StyleToggle({ active, icon: Icon, onClick }: any) {
  return (
    <Button 
      variant="outline" 
      onClick={onClick}
      className={cn("h-11 w-11 rounded-2xl border-white/10 transition-all", active ? "bg-primary border-primary text-white shadow-lg" : "bg-black/20 text-slate-500 hover:text-white hover:bg-white/5")}
    >
...
function PropertySection({ label, icon: Icon, children }: any) {
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 text-slate-400 px-2 group">
        <Icon className="w-4 h-4 group-hover:text-primary transition-colors" />
        <span className="font-headline font-bold text-[10px] uppercase tracking-[0.2em]">{label}</span>
      </div>
      <div className="bg-[#161625] p-6 rounded-[32px] border border-white/5 space-y-5 animate-in slide-in-from-top-2 duration-500">
        {children}
      </div>
    </div>
  );
...
function PropertyEditor({ block, products, onChange }: any) {
  switch (block.type) {
    case "header":
      return (
        <div className="space-y-5">
           <div className="space-y-3">
              <Label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest pl-1">Primary Title</Label>
              <Input value={block.content?.text || ""} onChange={(e) => onChange({ content: { ...block.content, text: e.target.value } })} className="rounded-xl bg-black/30 border-white/5 text-white h-12 focus:ring-primary focus:border-primary" />
           </div>
           <div className="space-y-3">
              <Label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest pl-1">Ranking</Label>
              <Select value={block.content?.level || "h2"} onValueChange={(v) => onChange({ content: { ...block.content, level: v } })}>
                <SelectTrigger className="rounded-xl h-12 bg-black/30 border-white/5 text-white"><SelectValue /></SelectTrigger>
                <SelectContent className="bg-[#1e1e2e] text-slate-300 border-white/10">
    ...
    case "paragraph":
      return (
        <div className="space-y-3">
           <Label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest pl-1">Text Narrative</Label>
           <Textarea value={block.content?.text || ""} onChange={(e) => onChange({ content: { ...block.content, text: e.target.value } })} className="rounded-2xl min-h-[160px] bg-black/30 border-white/5 text-white focus:ring-primary h-auto" />
        </div>
      );
    case "image":
      return (
    ...
    case "button":
      return (
        <div className="space-y-5">
           <div className="space-y-3">
              <Label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest pl-1">Action Label</Label>
              <Input value={block.content?.text || ""} onChange={(e) => onChange({ content: { ...block.content, text: e.target.value } })} className="rounded-xl h-12 bg-black/30 border-white/5 text-white" />
           </div>
           <div className="space-y-3">
              <Label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest pl-1">Gateway Link</Label>
              <Input value={block.content?.link || ""} onChange={(e) => onChange({ content: { ...block.content, link: e.target.value } })} className="rounded-xl h-12 bg-black/30 border-white/5 text-white" />
           </div>
        </div>
      );
    case "product-order-form":
      return (
        <div className="space-y-5">
           <div className="space-y-3">
              <Label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest pl-1">Featured Merchandise</Label>
              <Select value={block.content?.mainProductId || ""} onValueChange={(v) => onChange({ content: { ...block.content, mainProductId: v } })}>
                <SelectTrigger className="rounded-xl h-12 bg-black/30 border-white/5 text-white"><SelectValue placeholder="Select Product" /></SelectTrigger>
                <SelectContent className="bg-[#1e1e2e] text-slate-300 border-white/10">
           ...
           <div className="space-y-3">
              <Label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest pl-1">Logistics</Label>
